...
// Eigen::Matrix4f transformation_prev1;
// Eigen::Matrix4f transformation_prev2;
// Eigen::Matrix4f transformation_prev3;
...
void Callback(const nav_msgs::Odometry::ConstPtr &odom, const sensor_msgs::PointCloud2::ConstPtr &pcl_msg, const geometry_msgs::TransformStamped::ConstPtr &tf_msg, const geometry_msgs::TransformStamped::ConstPtr &tf_msg_wrt)
{
  ...
  transformation_prev = (transformation) * (transformation_prev);
  // transformation_prev2 = (transformation.inverse()) * (transformation_prev);
  // transformation_prev3 = (transformation_prev) * (transformation.inverse());
  ...
  // Eigen::Matrix4f error2;
  // Eigen::Matrix4f error3;
  // Eigen::Matrix4f error4;

  error = (ground_truth) * (transformation_prev.inverse());
  // error2 = (ground_truth) * (transformation_prev1.inverse());
  // error3 = (ground_truth) * (transformation_prev2.inverse());
  // error4 = (ground_truth) * (transformation_prev3.inverse());

  ROS_INFO("Error Matrix is,");
  ROS_INFO_STREAM(error);
  // ROS_INFO_STREAM(error2);
  // ROS_INFO_STREAM(error3);
  // ROS_INFO_STREAM(error4);
  double val = 180.0 / PI;
  geometry_msgs::Vector3 msg_angles;
  msg_angles.x = atan2(error(1,0),error(0,0)) * val;
  msg_angles.y = atan2(-error(2,0),sqrt(pow(error(2,1),2)+pow(error(2,2),2))) * val;
  msg_angles.z = atan2(error(2,1),error(2,2)) * val;
 
  ROS_INFO_STREAM(msg_angles);
  vis.publish(msg_angles);
  return;
}
